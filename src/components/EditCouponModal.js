...
const EditCouponModal = ({ coupon, onClose, onSuccess }) => {
  ...
  useEffect(() => {
    if (coupon) {
      setForm({
        ...
        delivery_type: coupon.delivery_type || 'both',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert('Error al actualizar cupón: ' + (error.response?.data?.error || error.message));
    } finally {
      setLoading(false);
    }
  };
    ...
    <Overlay onClick={onClose}>
      <Modal onClick={(e) => e.stopPropagation()}>
        ...
        <Form onSubmit={handleSubmit}>
          ...
          {coupon && coupon.current_uses > 0 && (
            <UsageInfo>
              <Label>Usos actuales</Label>
              <UsageText>
                Este cupón ha sido usado <strong>{coupon.current_uses}</strong> veces
                {coupon.max_uses && ` de un máximo de ${coupon.max_uses}`}
              </UsageText>
            </UsageInfo>
          )}
...
// Styled Components (los mismos que CreateCouponModal)
